...
# Display uplift model (simple heuristic)
# Base lift per display, modulated by account mix
account_lift = df['account'].map({
    'Tom Thumb': 0.08,
    'Kroger': 0.10,
    'Central Market': 0.07,
    'Whole Foods': 0.06,
...
void_penalty = 1.0 - (df['voids'] * 0.02)
void_penalty = void_penalty.clip(lower=0.6)

# Expected sales without displays
...
# Rollups
territory_summary = (
    df.groupby(['year', 'week', 'market'])
      .agg(goal=('goal', 'sum'),
           sales=('sales_volume', 'sum'),
           displays=('displays', 'sum'),
           pods=('pods', 'sum'),
           voids=('voids', 'sum'),
           uplift=('uplift_estimate', 'sum'))
      .reset_index()
)
...
account_summary = (
    df.groupby(['year', 'week', 'market', 'account'])
      .agg(goal=('goal', 'sum'),
           sales=('sales_volume', 'sum'),
           displays=('displays', 'sum'),
           pods=('pods', 'sum'),
...
print('Processed data saved to data/processed/ and rollups saved to data/outputs/.')
