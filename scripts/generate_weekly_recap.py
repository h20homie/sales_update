...
from utils import ensure_dirs, PROCESSED_DIR, RECAPS_DIR
...
latest_year = df[df['week']==latest_week]['year'].max()
...
# PDF
report_name = f"recap_{latest_year}-W{int(latest_week):02d}.pdf"
...
c.drawString(margin, height - margin - 16, f'Week {int(latest_week)}, {latest_year}')
...
c.save()
print(f'Wrote weekly recap to {report_path}')
